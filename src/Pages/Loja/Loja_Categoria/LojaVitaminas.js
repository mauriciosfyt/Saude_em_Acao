  ...
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        ...
        // 1. Busca os dados da API, filtrando por 'VITAMINAS'
        const data = await obterProdutos({ categoria: 'VITAMINAS' }); 
      ...
      <ScrollView>
        ...
        <View style={styles.produtosContainer}>
          {produtos.slice(0, 4).map((produto) => (
            // ALTERAÇÃO 1: Mudamos de View para TouchableOpacity e adicionamos onPress
            <TouchableOpacity 
                key={produto.id} 
            ...
            >
              ...
              {/* Botão Favorito (funciona independente) */}
        ...
        <View style={styles.produtosContainer}>
          {produtos.slice(4).map((produto) => (
            // ALTERAÇÃO 2: Mudamos de View para TouchableOpacity e adicionamos onPress
            <TouchableOpacity 
                key={produto.id} 
                style={styles.cardProduto}
                onPress={() => navigation.navigate('LojaProdutos', { produtoId: produto.id })}
