  ...
  useEffect(() => {
    const carregarProdutos = async () => {
      try {
        ...
        // 1. Busca os dados da API, filtrando por 'CAMISETAS'
        const data = await obterProdutos({ categoria: 'CAMISETAS' }); 
      ...
      <ScrollView>
        ...
        <View style={styles.produtosContainer}>
          {produtos.slice(0, 4).map((produto) => (
            // ALTERAÇÃO 1: View virou TouchableOpacity com onPress
            <TouchableOpacity 
                key={produto.id} 
            ...
            >
              ...
              {/* Botão de Favorito (continua funcionando isoladamente) */}
        ...
        <View style={styles.produtosContainer}>
          {produtos.slice(4).map((produto) => (
            // ALTERAÇÃO 2: View virou TouchableOpacity com onPress
            <TouchableOpacity 
                key={produto.id} 
                style={styles.cardProduto}
                onPress={() => navigation.navigate('LojaProdutos', { produtoId: produto.id })}
