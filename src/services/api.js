...
// Fluxo oficial conforme Rotas da API: POST http://98.92.159.66/api/auth/esqueci-senha/solicitar
// Usa URL absoluta para não depender de configuração de proxy/baseURL
export const solicitarEsqueciSenha = async (email) => {
  try {
    const response = await api.post('http://98.92.159.66/api/auth/esqueci-senha/solicitar', { 
...
export const validarCodigoEsqueciSenha = async (codigo) => {
  try {
    const response = await api.post('http://98.92.159.66/api/auth/esqueci-senha/validar-codigo', { codigo });
...
// Rota: POST http://98.92.159.66/api/auth/esqueci-senha/redefinir/{codigo}
// Envia apenas a nova senha no corpo da requisição
export const redefinirSenhaEsquecida = async (codigo, novaSenha) => {
  try {
    const url = `http://98.92.159.66/api/auth/esqueci-senha/redefinir/${encodeURIComponent(codigo)}`;
